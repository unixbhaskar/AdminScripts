...
FEED_TITLE=$(echo "${FEED_CONTENT}" | grep -v "xmlns:atom" | grep -o '<title>.*</title>' | head -n 1) | sed 
 -i's/\(<title>\|<\/title>\)//g'
FEED_LINK=$(echo "${FEED_CONTENT}"  | grep -v "xmlns:atom" | grep -o '<link>.*</link>' | head -n 1) | sed -i 
's/\(<link>\|<\/link>\)//g'
