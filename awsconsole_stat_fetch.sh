...
ec2din  --aws-access-key $AP_AWS_ACCESS_KEY_ID  -W $AP_AWS_SECRET_KEY_ID --url $EC2_URL --region $AP_REGION_NAME | grep 'INSTANCE' |  grep -v 'stopped' | cut  -f 2,7,6,14,15,17 | gawk -F=  ' { print $1 $2 $3 $4 $5 $6 }' > /home/$USER/AP_INSTANCE_DETAILS.txt


## Fetch details for the EU region
ec2din  --aws-access-key $EU_AWS_ACCESS_KEY_ID  -W $EU_AWS_SECRET_KEY_ID --url $EC2_URL --region $EU_REGION_NAME | grep 'INSTANCE' | grep -v 'stopped' | cut  -f 2,7,15,6,14,15,17 | gawk -F=  ' { print $1 $2 $3 $4 $5 $6 }' > /home/$USER/EU_INSTANCE_DETAILS.txt

## Fetch details for the US Region
ec2din  --aws-access-key $US_AWS_ACCESS_KEY_ID  -W $US_AWS_SECRET_KEY_ID --url $EC2_URL --region $US_REGION_NAME  | grep 'INSTANCE' | grep -v 'stopped' | cut  -f 2,7,15,6,14,15,17 | gawk -F= ' { print $1 $2 $3 $4 $5 $6 }' > /home/$USER/US_INSTANCE_DETAILS.txt
